...
export default async function VaryPage(props: {
  ...
  const vary = await findItems(id, searchParams);
  const varyStatic = await findItems(id);
  if (!vary) return notFound();
  const maxPrice = Math.max(
    ...varyStatic!.products.map((product) => product.price),
  );
  ...
  return (
    <Container className="page">
      <Title text={vary.name} size="lg" className="font-bold my-5" />
